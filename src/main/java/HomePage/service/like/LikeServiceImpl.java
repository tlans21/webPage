...
public class LikeServiceImpl implements LikeService {
    ...
    public Map<String, Integer> toggleLikeFromComment(Long commentId, Long userId, boolean isLike) {
        String targetType = LikeTargetType.COMMENT.getCode();
        System.out.println("test1");

        // 현재 사용자의 좋아요/싫어요 상태 확인0
        Like existingLike = likeMapper.findByTargetAndUser(targetType, commentId, userId);
        System.out.println("existingLike: " + existingLike);  // null인지 확인
        ...
        try {
            if (existingLike != null) {
                ...
                if (existingLike.isLike() == isLike) {
                    System.out.println("같은 타입 삭제 시도");
                    // 같은 타입(좋아요->좋아요 or 싫어요->싫어요)이면 삭제
                    int affectedRows = likeMapper.delete(targetType, commentId, userId);
                    System.out.println("삭제된 행 수: " + affectedRows);
                    // 삭제 후 카운트 갱신
                    int affectedRowsToReviewCount = likeMapper.updateReviewCounts(existingLike);
                    if (affectedRowsToReviewCount == 0) {
                ...
                System.out.println("새로운 좋아요/싫어요 생성 시도");
                // 없으면 새로 생성
                Like like = Like.createLike(LikeTargetType.COMMENT, commentId, userId, isLike);
            ...
            // 카운트 조회
            int likeCount = likeMapper.countLikes(targetType, commentId, true);
            int dislikeCount = likeMapper.countLikes(targetType, commentId, false);
            System.out.println("최종 카운트 - 좋아요: " + likeCount + ", 싫어요: " + dislikeCount);
